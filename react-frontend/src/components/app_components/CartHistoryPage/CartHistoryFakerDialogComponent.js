...
import cartHistoryFakerFactory from "./cartHistoryFakerFactory";
...
const CartHistoryFakerDialogComponent = (props) => {
...
client.service("voucher")
    ...
    const onRun = async () => {
        let fakeData = cartHistoryFakerFactory(props.user,fakerCount,userIDIds,voucherIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cartHistory").create(elem));
...
export default connect(mapState, mapDispatch)(CartHistoryFakerDialogComponent);
