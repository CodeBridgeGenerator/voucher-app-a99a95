...
const VoucherCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            title: _entity?.title,description: _entity?.description,expDate: _entity?.expDate,value: _entity?.value,points: _entity?.points,image: _entity?.image,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="expDate"  value={_entity?.expDate ? new Date(_entity?.expDate) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("expDate", new Date(e.target.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["expDate"]) ? (
              <p className="m-0" key="error-expDate">
                {error["expDate"]}
              </p>
            ) : null}
          </small>
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="value" className="w-full mb-3 p-inputtext-sm" value={_entity?.value} onChange={(e) => setValByKey("value", e.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["value"]) ? (
              <p className="m-0" key="error-value">
                {error["value"]}
              </p>
            ) : null}
          </small>
